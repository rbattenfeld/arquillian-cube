...
== Running the same Container Name concurrent

By default, Arquillian Cube checks if a docker container with the same container name is already started and stopps immediately if true. There are situations where this behavior
is not appropriate. For example, when developers working on the same project want to execute the same tests. Clearly, this is not nice when a developer has to wait until another developer
has finished his tests. 

The following solution makes the container name unique by adding an UUID identifier, and allows therefore concurrent executions. The solution is based on the gmaven-plugin plugin which allows to execute groovy
statements
       ...
       <execution>
		 ...
		 <configuration>
				...
				<source>
					...
					def uuid = UUIDGenerator.getInstance().generateRandomBasedUUID() <!--1-->
					project.properties.setProperty('arquillian.cube.UUID', uuid.toString()) <!--2-->
				</source>
			</configuration>
		</execution>
		<execution>
...
----
<1> A uuid identifier is created.
<2> The generated uuid is stored as project property with a property key 'arquillian.cube.UUID'. 
...
<profile>
	...
	<properties>
		<arquillian.launch>tomcat-${arquillian.cube.UUID}</arquillian.launch> <!--1-->
	</properties>
	<dependencies>
		<dependency>
			<groupId>org.jboss.arquillian.container</groupId>
...
----
<1> the arquillian.launch property is updated with the new unique container name.

[source, xml]
.arquillian.xml
----
<property name="dockerContainers">
   tomcat-${arquillian.cube.UUID}:
      image: tutum/tomcat:7.0
      exposedPorts: [8089/tcp]
      await:
        strategy: polling
...
<container qualifier="tomcat-${arquillian.cube.UUID}">
    <configuration>
        <property name="host">localhost</property>
